...
impl MCPServersEditPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let save_button = ctx.add_typed_action_view(|_| {
            ActionButton::new(warp_i18n::t!("settings-mcp-edit-save"), PrimaryTheme)
        ...
        let editing_disabled_banner = ctx.add_typed_action_view(|_| {
            Banner::new_without_close(BannerTextContent::plain_text(
                warp_i18n::t!("settings-mcp-edit-disabled-banner").to_string(),
            ))
    ...
    fn render_header(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let title = if self.server_card_item_id.is_none() {
            warp_i18n::t!("settings-mcp-edit-add-new-title").to_string()
        } else if let Some(name) = self.server_model.name() {
            warp_i18n::t!("settings-mcp-edit-edit-title-named", name = name).to_string()
        } else {
            warp_i18n::t!("settings-mcp-edit-edit-title-generic").to_string()
        };
        ...
        .with_tooltip(move || {
            ui_builder
                .tool_tip(warp_i18n::t!("settings-mcp-edit-tooltip-log-out").to_string())
                .build()
                .finish()
        })
        .build()
            ...
            Flex::column()
                ...
                .with_child(
                    Container::new(
                        Container::new(
                            Text::new(
                                warp_i18n::t!("settings-mcp-edit-format-json"),
                                ui_font_family,
                                font_size,
                            )
                            .finish(),
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(
                        warp_i18n::t!("settings-mcp-edit-error-contains-secrets").to_string(),
                    ),
                    window_id,
                    ctx,
                );
            });
            return Err(warp_i18n::t!("settings-mcp-edit-error-contains-secrets").to_string());
        }
    ...
    fn build_templatable_mcp_server_result_from_json(
        ...
        if parsed_templatable_mcp_servers.is_empty() {
            ...
            return Err(warp_i18n::t!("settings-mcp-edit-error-no-server-specified").to_string());
        }
        ...
        if parsed_templatable_mcp_servers.len() > 1 {
            ...
            ToastStack::handle(ctx).update(ctx, |toast_stack, ctx| {
                toast_stack.add_ephemeral_toast(
                    DismissibleToast::error(
                        warp_i18n::t!("settings-mcp-edit-error-multiple-servers").to_string(),
                    ),
                    window_id,
                    ctx,
                );
            ...
            return Err(warp_i18n::t!("settings-mcp-edit-error-multiple-servers").to_string());
        }
