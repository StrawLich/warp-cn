...
impl EnvironmentsPageView {
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let search_editor = Self::create_single_line_editor(
            warp_i18n::t!("settings-environments-list-search-placeholder"),
            ctx,
        );
        ...
        let pane_configuration = ctx.add_model(|_| {
            crate::pane_group::pane::PaneConfiguration::new(warp_i18n::t!(
    ...
    fn handle_update_manager_event(
            ...
            (&result.operation, &result.success_type)
        {
            ...
            if should_handle {
                ...
                self.show_success_toast(warp_i18n::t!("settings-environments-toast-updated"), ctx);
                ...
                if let Some(result_client_id) = &result.client_id {
                    if *result_client_id == pending_client_id {
                        self.show_success_toast(
                            warp_i18n::t!("settings-environments-toast-created"),
                            ctx,
                        );
                    }
                }
            ...
            if let Some(pending_env_id) = self.pending_delete_env_id.take() {
                ...
                if let Some(server_id) = &result.server_id {
                    if server_id.uid() == pending_env_id.uid() {
                        self.show_success_toast(
                            warp_i18n::t!("settings-environments-toast-deleted"),
                            ctx,
                        );
                    }
                }
        ...
        if matches!(&result.operation, ObjectOperation::MoveToDrive) {
            ...
            if matches!(result.success_type, OperationSuccessType::Success) {
                self.show_success_toast(warp_i18n::t!("settings-environments-toast-shared"), ctx);
            } else {
                self.show_error_toast(
                    warp_i18n::t!("settings-environments-toast-share-failed"),
                    ctx,
                );
            }
        ...
        match event {
            UpdateEnvironmentFormEvent::Created {
                ...
                let Some(owner) = owner else {
                    self.show_error_toast(
                        warp_i18n::t!("settings-environments-toast-create-not-logged-in"),
                        ctx,
                    );
                    return;
                };
            ...
            UpdateEnvironmentFormEvent::Updated {
                ...
                let Some(existing_env) = CloudAmbientAgentEnvironment::get_by_id(env_id, ctx)
                    ...
                    self.show_error_toast(
                        warp_i18n::t!("settings-environments-toast-save-not-exists"),
                        ctx,
                    );
                    return;
                };
        ...
        match action {
            ...
            EnvironmentsPageAction::ShareToTeam(env_id) => {
                ...
                let SyncId::ServerId(server_id) = *env_id else {
                    self.show_error_toast(
                        warp_i18n::t!("settings-environments-toast-share-not-synced"),
                        ctx,
                    );
                    return;
                };
...
impl EnvironmentsPageWidget {
    fn render_list_page(
        ...
        let title = Text::new(
            warp_i18n::t!("settings-environments-page-title"),
        ...
        let description = appearance
            .ui_builder()
            .paragraph(warp_i18n::t!("settings-environments-page-description"))
    ...
    fn render_no_matches_state(appearance: &Appearance) -> Box<dyn Element> {
        ...
        Container::new(
            Text::new(
                warp_i18n::t!("settings-environments-no-match"),
                appearance.ui_font_family(),
    ...
    fn render_scoped_section(
        ...
        let header = match list_scope {
            EnvironmentListScope::Personal => Self::render_overline_header(
                &warp_i18n::t!("settings-environments-section-personal"),
                appearance,
            ),
            EnvironmentListScope::Team => {
                let shared_by_text = UserWorkspaces::as_ref(app)
                    ...
                    .unwrap_or_else(|| {
                        warp_i18n::t!("settings-environments-section-shared-by-team-default")
                    });
    ...
    fn render_empty_state(
        ...
        let (github_button_label, github_button_enabled) = if dropdown_state.is_loading {
            (
                warp_i18n::t!("settings-environments-empty-button-loading"),
                false,
            )
        } else if dropdown_state.load_error_message.is_some() {
            (
                warp_i18n::t!("settings-environments-empty-button-retry"),
                true,
            )
        } else if dropdown_state.auth_url.is_some() {
            (
                warp_i18n::t!("settings-environments-empty-button-authorize"),
                true,
            )
        } else {
            (
                warp_i18n::t!("settings-environments-empty-button-get-started"),
                true,
            )
        };
        ...
        let launch_agent_label = warp_i18n::t!("settings-environments-empty-button-launch-agent");
        ...
        let github_row = Self::render_empty_state_row(
            ...
            EmptyStateRowConfig {
                icon: Icon::Github,
                title: warp_i18n::t_static!("settings-environments-empty-quick-setup-title"),
        ...
        let header = Text::new(
            warp_i18n::t!("settings-environments-empty-title"),
            appearance.ui_font_family(),
        ...
        let subheader = Text::new(
            warp_i18n::t!("settings-environments-setup-prompt"),
            appearance.ui_font_family(),
    ...
    fn render_environment_card(
        ...
        Hoverable::new(card_hover_mouse_state, move |state| {
            ...
            let mut details_parts = vec![format!(
                "{}: {}",
                warp_i18n::t!("settings-environments-card-image-prefix"),
                env_docker_image
            )];
            ...
            if !env_github_repos.is_empty() {
                ...
                details_parts.push(format!(
                    "{}: {}",
                    warp_i18n::t!("settings-environments-card-repos-prefix"),
                    repos_text
                ));
            }
            ...
            if !env_setup_commands.is_empty() {
                ...
                details_parts.push(format!(
                    "{}: {}",
                    warp_i18n::t!("settings-environments-card-setup-commands-prefix"),
                    commands_text
                ));
            }
            ...
            let view_runs_link = appearance
                .ui_builder()
                .link(
                    warp_i18n::t!("settings-environments-card-link-view-runs"),
                    None,
            ...
            if should_render_share_button {
                ...
                let share_button_element = if is_card_hovered {
                    ...
                    .with_tooltip(move || {
                        share_ui_builder
                            .tool_tip(warp_i18n::t!("settings-environments-card-tooltip-share"))
                            .build()
                            .finish()
                    })
                    .build()
            ...
            if is_card_hovered {
                edit_button = edit_button.with_tooltip(move || {
                    edit_ui_builder
                        .tool_tip(warp_i18n::t!("settings-environments-card-tooltip-edit"))
                        .build()
                        .finish()
                });
            }
...
impl BackingView for EnvironmentsPageView {
    ...
    fn render_header_content(
        ...
        HeaderContent::simple(warp_i18n::t!("settings-environments-page-title"))
    }
